...
#' @export
apollo_modelAveraging <- function(lc_settings, apollo_inputs, functionality){
  modelType = 'LC'
  ...
  if(functionality==("gradient")){
    # SH added 24 Oct
    for(i in 1:length(inClassProb)){
      inClassProb[[i]]$like=inClassProb[[i]]
      inClassProb[[i]]$grad=list()
      for(k in 1:nParam) inClassProb[[i]]$grad[[k]]=0
    }
    # Initial checks
