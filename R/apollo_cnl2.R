...
#' @export
apollo_cnl2 <- function(cnl_settings, functionality){
  ...
  if( !is.null(apollo_inputs[[paste0(cnl_settings$componentName, "_settings")]]) && (functionality!="preprocess") ){
    ...
    cnl_settings$probs_CNL <- function(cnl_settings, all=FALSE){
      ...
      # create nest availability variables
      cnl_settings$nestAvail <- list()
      for(t in 1:nests){
        cnl_settings$nestAvail[[t]] = 0
        for(j in 1:cnl_settings$nAlt){
          cnl_settings$nestAvail[[t]] = cnl_settings$nestAvail[[t]] + cnl_settings$cnlStructure[t,j]*cnl_settings$avail[[j]]
        }
      }
      ...
      for(t in 1:nests){
        within_nest_G[[t]] = 0
        for(j in 1:cnl_settings$nAlt){
          within_nest_G[[t]] = within_nest_G[[t]] + (cnl_settings$cnlStructure[t,j]*cnl_settings$V[[cnl_settings$altnames[j]]])^(1/cnl_settings$cnlNests[[t]])
        }
        cnl_settings$G = cnl_settings$G + within_nest_G[[t]]^cnl_settings$cnlNests[[t]]
      }
      # take first derivatives of G (to get Gi)
      Gi = list()
      for(j in 1:cnl_settings$nAlt){
        Gi[[j]] = 0
        for(t in 1:nests){ ### can simplify this still to only loop over those nests that include alt j
          tmp = cnl_settings$cnlNests[[t]]*within_nest_G[[t]]^(cnl_settings$cnlNests[[t]]-1)*1/cnl_settings$cnlNests[[t]]*(cnl_settings$cnlStructure[t,j]*cnl_settings$V[[cnl_settings$altnames[j]]])^(1/cnl_settings$cnlNests[[t]]-1)*cnl_settings$cnlStructure[t,j]
          tmp = apollo_setRows(tmp, !cnl_settings$avail[[j]], 0)
          #tmp = apollo_setRows(tmp, (within_nest_G[[t]]==0), 0) ### expand to cube condition etc
          tmp = apollo_setRows(tmp, (cnl_settings$nestAvail[[t]]==0), 0) ### expand to cube condition etc
          tmp  = apollo_setRows(tmp, (cnl_settings$cnlStructure[t,j]*cnl_settings$V[[cnl_settings$altnames[j]]]==0),0)
          Gi[[j]] = Gi[[j]] + tmp
        } 
      }
      rm(within_nest_G)
      ...
      Palts = list()
      for(j in 1:cnl_settings$nAlt){
        Palts[[j]]=cnl_settings$V[[cnl_settings$altnames[j]]]*Gi[[j]]/cnl_settings$G
      }
