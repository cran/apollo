context("Test apollo_mnl")
library(apollo)

test_that("length of MNL likelihood is same than database", {
  ...
  lik <- apollo_mnl(alternatives, avail, choiceVar, V)

  expect_equal(length(lik), nrow(x))
})

test_that("no NAs in MNL likelihood", {
  data(apollo_modeChoiceData)
  x            = database
  b            = list(asc_1=0, asc_2=0, asc_3=0, asc_4=0, tt=0, tc=0, acc=0)
  alternatives = c(car=1, bus=2, air=3, rail=4)
